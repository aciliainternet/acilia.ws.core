...
function cancelCrop(event) {
  ...
  document.querySelector(
    `.ws-cropper_modal[data-id="${dataId}"] .ws-cropper_crop img`
  ).src = "";
  document.querySelector(
    `.ws-cropper_modal[data-id="${dataId}"]`
  ).dataset.croppIndex = 0;
...
function saveCrop(id) {
  try {
    ...
    if (
      ...
      document
        .querySelector(
          `.js-open-modal.js-not-on-preview[data-id-asset-component="${id}"]`
        )
    ...
    document
      ...
      .forEach((elm) => {
        ...
        if (element.querySelector("img")) {
          element.querySelector("img").src = fieldData.cropper
            .getCroppedCanvas()
            .toDataURL();
...
function showCropper(elm, cropperIndex) {
  try {
    ...
    ).innerText = cropperConfig.minimums.width;
    document.querySelector(
...
function nextCrop(event) {
  ...
  const index = parseInt(
    document.querySelector(`.ws-cropper_modal[data-id="${id}"]`).dataset
      .croppIndex,
    10
  );
...
async function initCropper(event, loaderContainer) {
  ...
  const modalCroppper = document.querySelector(
    ...
    if (!imgValidator.isValid) {
      ...
      if (error) {
        const errorMsg = error
          .replace("%width%", imgValidator.minWidth)
          .replace("%height%", imgValidator.minHeight);
        elm.value = "";
